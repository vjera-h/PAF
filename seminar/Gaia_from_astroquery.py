...
job = Gaia.launch_job_async("SELECT ra, dec, phot_g_mean_mag FROM gaiadr3.gaia_source WHERE phot_g_mean_mag <= 10.0")
# SELECT - izdvaja ra, dec, phot_g_mean_mag jer su nam ti potrebni
# FROM gaiadr3.gaia_source -> data release 3
# WHERE phot_g_mean_mag <= 10.0 -> ograničava rezultate na najsvjetlije

result = job.get_results() # uzima ono što se tražilo iz launch_job_async

# pretvaranje u np.array
ra_deg = np.array(result['ra'])
dec_deg = np.array(result['dec'])
mag = np.array(result['phot_g_mean_mag'])
